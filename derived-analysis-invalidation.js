(() => {
  ...
  const DERIVED_FIELDS = [
    ...
    "auctionRecords10y","auctionRecordCount10y","auctionMarketSnapshot","auctionStrictMatches10y","marketMedian","marketCurrency","valuationCurrency","priceRange","valuationConfidence","valuationUpdatedAt","estimateLow","estimateHigh","estimatedPrice","estimatedValue","marketValue","valuation","priceEstimate","estimate",
  ...
  function gateCatalogEvidence(detail) {
    ...
    const explicitCandidateReference = clean(output.kopickiCandidateReference);
    const explicitCandidateRarity = clean(output.kopickiCandidateRarity).toUpperCase().replace(/\s+/g, "");
    const explicitCandidateBasis = clean(output.kopickiCandidateBasis);
    ...
    const diagnostics = Array.isArray(output.diagnosticFeatures)
      ? output.diagnosticFeatures.map(clean).filter((value) => value && !isUnknown(value))
      : [];
    const fingerprintCount = strongFingerprintCount(output);
    const rarityValid = !rarity || /^(?:R|R[1-8])$/.test(rarity);
    const diagnosticSupport =
      diagnostics.length >= CATALOG_MIN_DIAGNOSTICS ||
      fingerprintCount >= CATALOG_MIN_FINGERPRINT_FEATURES;
    const strongVariantEvidence =
      variant &&
      !isUnknown(variant) &&
      confidence >= CATALOG_MIN_CONFIDENCE &&
      diagnosticSupport;
    const catalogAccepted = Boolean(
      reference && !isUnknown(reference) && rarityValid && strongVariantEvidence,
    );
    if (catalogAccepted) {
    ...
    const candidateReference =
      explicitCandidateReference && !isUnknown(explicitCandidateReference)
        ? explicitCandidateReference
        : reference && !isUnknown(reference)
          ? reference
          : "";
    const proposedCandidateRarity = explicitCandidateRarity || rarity;
    const candidateRarity = /^(?:R|R[1-8])$/.test(proposedCandidateRarity)
      ? proposedCandidateRarity
      : "";
    if (candidateReference) {
      output.catalogCandidate = {
        reference: candidateReference,
        rarity: candidateRarity,
        basis: explicitCandidateBasis,
        confidence,
        diagnosticFeatureCount: diagnostics.length,
        strongFingerprintFeatureCount: fingerprintCount,
      };
      output.warnings = [
        ...(Array.isArray(output.warnings) ? output.warnings : []),
        "Numeru Kopickiego i stopnia rzadkości nie pokazano jako potwierdzonych. Kandydat wymaga cech konkretnej odmiany.",
      ];
    }
