...
function rulerFromEvidence(observations = {}) {
  return explicitRuler(evidenceLegend(observations)) || canonicalRuler(observations.rulerReading);
}
...
export function retrieveCandidatesWithEngines(observations, candidates, inputMeasurements = {}, options = {}) {
  ...
  const applyHardEvidence = (input) => input.filter((entry) => {
    if (year) {
      ...
      const stableIdentityMatch = ["ruler", "mint", "metal"]
        .every((engine) => entry.engines.includes(engine));
      const hasLegalVisualReference = Array.isArray(entry.candidate?.images) &&
        entry.candidate.images.some((url) => /^https:\/\//i.test(clean(url)));
      const visualYearRival = !matchingYear && candidateYear &&
        stableIdentityMatch && hasLegalVisualReference;
      if (!matchingYear && !supportedYearless && !visualYearRival) {
        diagnostics.chronologyEngine.rejected += 1;
        return false;
      }
      if (visualYearRival) {
        entry.points = Math.max(0, entry.points - 4);
        diagnostics.chronologyEngine.retainedAsVisualRival += 1;
      }
    }
