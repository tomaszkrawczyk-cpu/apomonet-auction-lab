...
function canonicalNominal(value) {
  ...
  if (/\b(?:lub|albo|or|uncertain|nie ustalono|brak)\b|\?/.test(source)) return "";
  return source
...
function explicitRuler(value) {
  ...
  if (/\b(?:ioan\w*|john|johann|jan)\b/.test(ruler) && /\b(?:iii|3|sobies\w*)\b/.test(ruler)) return "jan iii sobieski";
  if (/\b(?:stanisla\w*)\b/.test(ruler) && /\b(?:august\w*|poniatow\w*)\b/.test(ruler)) return "stanislaw august poniatowski";
  if (/\b(?:hludov\w*|hlvdov\w*|ludovic\w*|ludwik)\b/.test(ruler)) return "ludwik";
  return "";
}
...
function canonicalMint(value) {
  ...
  const aliases = [
    ...
    [/\b(?:bromberg|bydgoszcz)\b/, "bydgoszcz"], [/\b(?:thorn|thorvnia|thorvn|thorun|torun)\b/, "torun"],
...
function canonicalShape(value) {
  ...
  if (/\b(?:square|kwadrat\w*|klip\w*|romb\w*|diamond\w*|lozenge\w*|czworokat\w*)\b/.test(shape)) {
    return "square-klippe";
  }
  if (/\b(?:round|okrag\w*|kolist\w*|circle)\b/.test(shape)) return "round";
  return shape;
}
...
function mintFromEvidence(observations = {}) {
  ...
  const legendText = clean([
    evidenceLegend(observations),
...
function candidateMatchesPartialYear(candidate, observedValue) {
  const prefix = clean(observedValue).match(/\b(9\d|1\d{2}|20\d)\s*(?:\?|x)/i)?.[1] || "";
  return Boolean(prefix && candidateYearKeys(candidate).some((value) => value.startsWith(prefix)));
}
...
const CONTROLLED_CONFUSION_FAMILIES = Object.freeze([
  ...
  "counterstamped-host-coin",
]);

function hasCounterstampEvidence(value) {
  const text = normalized(Array.isArray(value) ? value.flat(Infinity).join(" ") : value);
  return /\b(?:counterstamp\w*|countermark\w*|contramark\w*|kontrmark\w*|kontrsygn\w*|kontrstempl\w*)\b/.test(text) ||
    /\b(?:ukoronowan\w*|crowned)\b.{0,30}\b(?:monogram\w*\s*)?sa\b/.test(text);
}

function tokens(value) {
  const result = normalized(Array.isArray(value) ? value.flat(Infinity).join(" ") : value)
    .split(/\s+/)
    .filter((token) => token.length >= MIN_TOKEN_LENGTH && !STOPWORDS.has(token))
    .slice(0, 64);
  if (result.some((token) => /^(?:thorvnia|thorvn|thorun)$/.test(token))) result.push("torun");
  if (result.some((token) => /^(?:obron\w*|oblez\w*|siege\w*)$/.test(token))) result.push("siege");
...
function controlledFamilyConflict(observations = {}, ranking = {}, inputMeasurements = {}) {
  ...
  const counterstampFamily = CONTROLLED_CONFUSION_FAMILIES[6];
  const observedCounterstamp = hasCounterstampEvidence([
  ...
  if (observedCounterstamp && !activeFamilies.includes(counterstampFamily)) activeFamilies.push(counterstampFamily);
  if (top && observedCounterstamp && !hasCounterstampEvidence([
    top.candidate.title,
    top.candidate.diagnosticMarkers,
    top.candidate.obverseLegend,
    top.candidate.reverseLegend,
  ])) {
    return {
      blocked: true,
      activeFamilies,
      reason: "Widoczna kontrmarka może zmieniać tożsamość monety gospodarza; brak zweryfikowanego typu kontrmarkowanego w krótkiej liście wymaga zatrzymania analizy.",
    };
  }
...
function buildIndexes(candidates) {
  ...
  const indexes = {
    year: new Map(), nominal: new Map(), ruler: new Map(), mint: new Map(), metal: new Map(), shape: new Map(), token: new Map(),
  };
  candidates.forEach((candidate, index) => {
    ...
    addIndex(indexes.shape, canonicalShape(candidate.shape), index);
    for (const token of tokens([
      ...
      candidate.issuer, candidate.shape, candidate.objectKind,
...
function consolidateSpecimens(entries) {
  ...
  return [...groups.values()].map((group) => {
    ...
    // One basic identity may be represented by several legal museum
    // specimens and design variants.  Keep their images for the independent
    // visual gate instead of silently reducing the whole group to whichever
    // specimen happened to sort first.  The bounded visual allocator decides
    // how many of these images can be sent in one request.
    const visualReferenceImages = [...new Set(group.flatMap((entry) =>
      Array.isArray(entry.candidate?.images) ? entry.candidate.images : [],
    ...
    return {
      ...
      candidate: {
        ...
        visualReferenceSpecimenCount: group.filter((entry) =>
          Array.isArray(entry.candidate?.images) && entry.candidate.images.length > 0,
        ).length,
      },
...
export function retrieveCandidatesWithEngines(observations, candidates, inputMeasurements = {}, options = {}) {
  ...
  const shape = canonicalShape(observations?.shape);
  ...
  if (shape) diagnostics.fieldEngine.matches += vote(votes, indexes, "shape", shape, 8, "shape");
  const observedTokens = tokens([
    observations?.portrait, observations?.heraldry, observations?.mintMarks,
    observations?.rulerReading, observations?.obverseLegendFragments,
    observations?.reverseLegendFragments, observations?.historicalTypeHypothesis,
    observations?.historicalEvidence, observations?.countryReading,
    observations?.issuerReading, observations?.shape, observations?.objectKind,
  ]);
  ...
  const applyHardEvidence = (input) => {
    ...
    const accepted = input.filter((entry) => {
      ...
      let interwarKlippeVisualRival = false;
      if (year) {
        ...
        if (!matchingYear && !supportedYearless) {
          ...
          const candidateEvidence = normalized([
            entry.candidate?.title,
            entry.candidate?.diagnosticMarkers,
            entry.candidate?.obverseLegend,
            entry.candidate?.reverseLegend,
          ].flat().filter(Boolean).join(" "));
          const municipalSiegeMatch = Boolean(
            candidateYear &&
            hasLegalVisualReference &&
            entry.engines.includes("mint") &&
            observedTokens.includes("siege") &&
            /\b(?:oblez\w*|obron\w*|siege\w*)\b/.test(candidateEvidence)
          );
          interwarKlippeVisualRival = Boolean(
            candidateYear &&
            Number(candidateYear) >= 1920 &&
            Number(candidateYear) <= 1939 &&
            hasLegalVisualReference &&
            shape === "square-klippe" &&
            canonicalShape(entry.candidate?.shape) === "square-klippe" &&
            clean(entry.candidate?.periodId || entry.candidate?.period) ===
              "second-republic-and-war" &&
            observedTokens.includes("polska") &&
            observedTokens.some((token) => /^(?:zaglow\w*|statek|ship|sailing)$/.test(token))
          );
          visualYearRival = Boolean(
            candidateYear &&
            hasLegalVisualReference &&
            (stableIdentityMatch || municipalSiegeMatch || interwarKlippeVisualRival)
          );
          if (!visualYearRival) {
            diagnostics.chronologyEngine.rejected += 1;
            return false;
          }
          if (municipalSiegeMatch) {
            // A city siege issue often has no portrait/ruler to anchor the
            // metadata.  THORVNIA + the siege design + a legal reference is
            // enough to keep a different-year record in the independent
            // image comparison, never enough to select it automatically.
            entry.points += 14;
            if (!entry.engines.includes("municipal-siege-year-rival")) {
              entry.engines.push("municipal-siege-year-rival");
            }
          } else if (interwarKlippeVisualRival) {
            // A one-digit OCR error must not discard the small, bounded set
            // of interwar Polish pattern klippes before the independent image
            // gate can compare their fundamentally different designs.
            entry.points += 14;
            if (!entry.engines.includes("interwar-klippe-visual-rival")) {
              entry.engines.push("interwar-klippe-visual-rival");
            }
          } else {
      ...
      if (nominal && candidateNominal && nominal !== candidateNominal) {
        const candidateShape = canonicalShape(entry.candidate.shape);
        const exactDistinctiveShapeAndYear = Boolean(
          ((year
            ? candidateMatchesYear(entry.candidate, year)
            : candidateMatchesPartialYear(entry.candidate, observations?.yearReading)) ||
            interwarKlippeVisualRival) &&
          shape === "square-klippe" &&
          candidateShape === shape,
        );
        if (!exactDistinctiveShapeAndYear) {
          diagnostics.denominationEngine.rejected += 1;
          return false;
        }
        // A blurred 1/5/10 reading must not remove the only same-year klippe
        // before the independent image comparison can inspect it.
        entry.points += 6;
        if (!entry.engines.includes("shape-year-rival")) entry.engines.push("shape-year-rival");
      }
      ...
      if (visualYearRival) {
        const key = identityKey(entry.candidate);
        const group = visualYearRivals.get(key) || [];
        group.push(entry);
        visualYearRivals.set(key, group);
        return false;
      }
      return true;
    });
    diagnostics.chronologyEngine.retainedAsVisualRival += visualYearRivals.size;
    return [...accepted, ...[...visualYearRivals.values()].flat()];
  };
