...
const MAX_REFERENCE_IMAGES_TOTAL = 10;
...
function usableImages(candidate, limit = MAX_IMAGES_PER_TYPE) {
  const pool = Array.isArray(candidate?.visualReferenceImages) && candidate.visualReferenceImages.length
    ? candidate.visualReferenceImages
    : candidate?.images;
  return [...new Set(Array.isArray(pool) ? pool : [])]
    .map(clean)
    .filter((url) => /^https:\/\//i.test(url))
    .slice(0, Math.max(1, Math.min(Number(limit) || MAX_IMAGES_PER_TYPE, MAX_IMAGES_PER_TYPE)));
}
...
function isMunicipalSiegeYearRival(item, minimumScore) {
  const score = Number(item?.score) || 0;
  const title = normalized(item?.candidate?.title);
  const hasYearConflict = Array.isArray(item?.selectionConflicts) &&
    item.selectionConflicts.some((conflict) => /roku|year/i.test(clean(conflict)));
  return score >= Math.max(30, minimumScore - 5) &&
    hasYearConflict &&
    /\b(?:oblez\w*|siege\w*)\b/.test(title) &&
    Boolean(clean(item?.candidate?.mint));
}
...
export function visualReferenceShortlist(ranked, options = {}) {
  ...
  const minimumScore = Number.isFinite(Number(options.minimumScore))
    ...
    : MIN_METADATA_SCORE;
  const selected = (Array.isArray(ranked?.ranked) ? ranked.ranked : [])
    .filter(
      (item) =>
        (Number(item?.score) >= minimumScore || isMunicipalSiegeYearRival(item, minimumScore)) &&
    ...
    .slice(0, limit);
  const pools = selected.map((item) => usableImages(item.candidate));
  const allocated = selected.map(() => []);
  let remaining = MAX_REFERENCE_IMAGES_TOTAL;
  // Give every displayed type one image first, then a second side.  Spare
  // slots go to the strongest identity group so alternative museum variants
  // can be inspected without increasing the total request budget.
  for (let pass = 0; pass < 2 && remaining > 0; pass += 1) {
    for (let index = 0; index < pools.length && remaining > 0; index += 1) {
      if (pools[index][pass]) {
        allocated[index].push(pools[index][pass]);
        remaining -= 1;
      }
    }
  }
  for (let index = 0; index < pools.length && remaining > 0; index += 1) {
    for (const image of pools[index].slice(allocated[index].length)) {
      if (remaining <= 0) break;
      allocated[index].push(image);
      remaining -= 1;
    }
  }
  return selected.map((item, visualRank) => ({
      ...item,
      visualRank: visualRank + 1,
      referenceImages: allocated[visualRank],
    }));
}
...
export function shouldCompareVisualReferences(ranked, shortlist = visualReferenceShortlist(ranked)) {
  ...
  if (!top) return false;
  const evidenceGap = Number.isFinite(Number(ranked?.gap))
    ? Number(ranked.gap)
    : Number(top.score) - Number(ranked?.ranked?.[1]?.score || 0);
  // A single high-margin evidence leader is safe to show at its deterministic
  // evidence confidence without paying for extra catalogue-image confirmation.
  // This gate never raises confidence. Ambiguous leaders (such as the
  // user-reported Batory 61 vs 59 case) still go through visual rejection.
  if (
    Number(top.score) >= 65 &&
    evidenceGap >= 25 &&
    (!Array.isArray(top.hardConflicts) || top.hardConflicts.length === 0)
  ) return false;
  // Stage 1 only needs the basic identity. If every referenced record agrees
  // on country, issuing authority, year and denomination, a slow second model
  // call can only distinguish material/mint variants that belong to Stage 2.
  const contenders = shortlist.filter((item) =>
    Number(item?.score) >= Number(top.score) - 8,
  );
  const sharedBasicIdentity = contenders.length > 1 && ["country", "ruler", "year", "nominal"]
    .every((field) => {
      const values = contenders.map((item) => normalized(item?.candidate?.[field]));
      return values.every(Boolean) && new Set(values).size === 1;
    });
  if (sharedBasicIdentity && Number(top.score) >= 70) return false;
...
function normalizedComparison(raw, allowedIds) {
  ...
  return {
    ...
    decisiveFeatures: Array.isArray(raw?.decisiveFeatures)
      ...
      : [],
    conflictingFields: Array.isArray(raw?.conflictingFields)
      ? raw.conflictingFields
        .map(clean)
        .filter((field) => ["country", "issuer", "ruler", "depictedPerson", "year", "nominal", "mint", "metal", "design"].includes(field))
        .slice(0, 6)
      : [],
 ...
 */
function observationSupportsRejectedCandidate(field, observations, candidate) {
  const observationFields = {
    country: "countryReading",
    issuer: "issuerReading",
    ruler: "rulerReading",
    depictedPerson: "depictedPersonReading",
    year: "yearReading",
    nominal: "denominationReading",
    mint: "mintReading",
    metal: "metalAppearance",
  };
  const observed = normalized(observations?.[observationFields[field]]);
  const expected = normalized(candidate?.[field]);
  if (!observed || !expected) return false;
  if (field === "year") {
    const observedYears = [...observed.matchAll(/\b(?:1\d{3}|20\d{2}|9\d{2})\b/g)].map((match) => match[0]);
    const expectedYears = [...expected.matchAll(/\b(?:1\d{3}|20\d{2}|9\d{2})\b/g)].map((match) => match[0]);
    return observedYears.some((year) => expectedYears.includes(year));
  }
  return observed === expected || observed.includes(expected) || expected.includes(observed);
}

export function resolveVisualComparison(raw, shortlist, observations = null) {
  ...
  const rejected = comparisons.filter((comparison) =>
    !comparison.sameType && comparison.contradictions.length > 0,
  );
  const blockedIdentityFields = selected
    ? []
    : [...new Set(rejected.flatMap((comparison) => {
      const candidate = (shortlist || []).find(
        (item) => item?.candidate?.id === comparison.candidateId,
      )?.candidate;
      return comparison.conflictingFields.filter((field) =>
        !observations || observationSupportsRejectedCandidate(field, observations, candidate),
      );
    }))]
      .filter((field) => field !== "design");
  return {
    ...
    comparisons,
    rejectedCandidateIds: rejected.map((comparison) => comparison.candidateId),
...
export const visualRecognitionPolicy = Object.freeze({
  ...
  maxReferenceImagesTotal: MAX_REFERENCE_IMAGES_TOTAL,
