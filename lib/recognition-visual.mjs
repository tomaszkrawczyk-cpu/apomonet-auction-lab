...
export function shouldCompareVisualReferences(ranked, shortlist = visualReferenceShortlist(ranked)) {
  ...
  // A single high-margin evidence leader is safe to show as a partial Stage-1
  // identity without paying for catalogue confirmation. It is deliberately
  // not promoted to a confirmed candidate here. Ambiguous leaders (such as
  // the user-reported Batory 61 vs 59 case) still go through visual rejection.
  if (
    !ranked?.selected &&
    Number(top.score) >= 65 &&
    evidenceGap >= 25 &&
...
export function resolveVisualComparison(raw, shortlist) {
  ...
  const blockedIdentityFields = selected
    ? []
    : [...new Set(rejected.flatMap((comparison) => comparison.conflictingFields))]
