...
function explicitMetal(value) {
  ...
  if (/\b(zlot\w*|zolty\w*|zoltozlot\w*|zlocist\w*|zlotaw\w*|gold\w*|yellow\w*|avr|aure\w*)\b/.test(metal)) return "zloto";
...
export function adjudicateRecognition(raw, candidates, inputMeasurements = {}) {
  ...
  const nonCoinKinds = new Set(["medal", "token", "zeton", "copy", "replica", "replika", "cast", "odlew"]);
  if (nonCoinKinds.has(observedObjectKind)) {
    const cautionByKind = {
