...
import evidenceSignatures from "../data/recognition/evidence-signatures-v1.json" with { type: "json" };
...
export function needsMedievalSpecialistReview(observations = {}) {
  const context = normalized([
    observations.objectKind,
    observations.countryReading,
    observations.periodReading,
    observations.historicalTypeHypothesis,
    observations.historicalEvidence,
    observations.portrait,
    observations.heraldry,
    observations.obverseLegendFragments,
    observations.reverseLegendFragments,
  ].flat().filter(Boolean).join(" "));
  const medievalContext = /\b(?:sredniowie\w*|karoling\w*|caroling\w*|christiana|xpistiana|hlvdov\w*|hludov\w*)\b/.test(context);
  const medievalMotifs = /\b(?:krzyz|cross)\b/.test(context) &&
    /\b(?:swiatyn\w*|fasad\w*|temple|church)\b/.test(context);
  if (!medievalContext && !medievalMotifs) return false;
  const rulerEvidence = clean([
    observations.rulerReading,
    observations.obverseLegendFragments,
    observations.reverseLegendFragments,
  ].flat().filter(Boolean).join(" "));
  const historicalEvidence = Array.isArray(observations.historicalEvidence)
    ? observations.historicalEvidence.map(clean).filter(Boolean)
    : [];
  const confidence = Number(observations.historicalTypeConfidence) || 0;
  const rulerKey = explicitRuler(rulerEvidence);
  const rulerSpecificEnough = Boolean(
    rulerKey &&
    (rulerKey !== "ludwik" || /\bpobozn\w*\b/.test(normalized(observations.rulerReading)))
  );
  return !rulerSpecificEnough || historicalEvidence.length < 3 || confidence < 80;
}

export function mergeMedievalSpecialistObservations(base = {}, review = {}) {
  const evidence = Array.isArray(review.historicalEvidence)
    ? review.historicalEvidence.map(clean).filter(Boolean).slice(0, 6)
    : [];
  const confidence = Math.max(0, Math.min(95, Number(review.historicalTypeConfidence) || 0));
  if (confidence < 75 || evidence.length < 2) {
    return { observations: { ...base }, improvedFields: [] };
  }
  const next = { ...base };
  const improvedFields = [];
  let verifiedRuler = false;
  const use = (key, options = {}) => {
    const value = clean(review[key]);
    if (!value || unknownObservation(value)) return;
    if (options.ruler) {
      const rulerKey = explicitRuler(value);
      const previousRulerKey = explicitRuler(base.rulerReading);
      const proof = normalized([
        evidence,
        review.obverseLegendFragments,
        review.reverseLegendFragments,
      ].flat().filter(Boolean).join(" "));
      if (!rulerKey) return;
      if (previousRulerKey && previousRulerKey !== rulerKey) return;
      if (rulerKey === "ludwik" && !/\b(?:hlvdov\w*|hludov\w*|ludovic\w*)\b/.test(proof)) return;
      verifiedRuler = true;
    }
    const previous = clean(base[key]);
    if (!unknownObservation(previous) && options.onlyWhenMissing) return;
    if (previous === value) return;
    next[key] = value;
    improvedFields.push(key);
  };
  use("countryReading");
  use("rulerReading", { ruler: true });
  if (verifiedRuler) use("issuerReading", { onlyWhenMissing: true });
  use("periodReading");
  use("historicalTypeHypothesis");
  const mergeArray = (key, limit) => {
    const values = [...new Set([
      ...(Array.isArray(base[key]) ? base[key] : []),
      ...(Array.isArray(review[key]) ? review[key] : []),
    ].map(clean).filter(Boolean))].slice(0, limit);
    if (!values.length) return;
    next[key] = values;
    if (values.length > (Array.isArray(base[key]) ? base[key].length : 0)) improvedFields.push(key);
  };
  mergeArray("historicalEvidence", 6);
  mergeArray("obverseLegendFragments", 8);
  mergeArray("reverseLegendFragments", 8);
  mergeArray("heraldry", 6);
  mergeArray("mintMarks", 6);
  next.historicalTypeConfidence = Math.max(
    Number(base.historicalTypeConfidence) || 0,
    confidence,
  );
  return { observations: next, improvedFields: [...new Set(improvedFields)] };
}

/**
 * Request one evidence-only legend reread when a source-backed signature is
 * close enough to justify it. A place name on its own is deliberately
 * insufficient: the observation must also contain a registry-defined visual
 * feature, such as the panorama that accompanies THORVNIA on the siege type.
 */
export function needsEvidenceSignatureReview(observations = {}) {
  const legendText = [
    ...(Array.isArray(observations.obverseLegendFragments)
      ? observations.obverseLegendFragments
      : []),
    ...(Array.isArray(observations.reverseLegendFragments)
      ? observations.reverseLegendFragments
      : []),
  ].map(clean).filter(Boolean).join(" ");
  const legendTokens = normalized(legendText)
    .split(/\s+/)
    .map(epigraphicToken)
  ...
  const visualText = normalized([
    observations.portrait,
    observations.historicalTypeHypothesis,
    ...(Array.isArray(observations.historicalEvidence)
      ? observations.historicalEvidence
      : []),
    ...(Array.isArray(observations.heraldry) ? observations.heraldry : []),
  ].map(clean).filter(Boolean).join(" "));

  return (Array.isArray(evidenceSignatures.signatures)
    ? evidenceSignatures.signatures
    : []).some((signature) => {
    const trigger = signature?.reviewTrigger;
    if (!trigger) return false;
    const requiredLegendFragments = Array.isArray(trigger.requiredLegendFragments)
      ? trigger.requiredLegendFragments.map(clean).filter(Boolean)
      : [];
    if (!requiredLegendFragments.length || !requiredLegendFragments.every((fragment) =>
      fragmentVisible(fragment, legendTokens))) return false;
    const visualTerms = Array.isArray(trigger.visualTerms)
      ? trigger.visualTerms.map(normalized).filter(Boolean)
      : [];
    const visibleVisualTerms = visualTerms.filter((term) => visualText.includes(term));
    return visibleVisualTerms.length >= Math.max(1, Number(trigger.minimumVisualTerms) || 1);
  });
}

export function mergeEvidenceSignatureReview(base = {}, review = {}) {
  const next = { ...base };
  const improvedFields = [];
  for (const key of ["obverseLegendFragments", "reverseLegendFragments"]) {
    const previous = Array.isArray(base[key]) ? base[key].map(clean).filter(Boolean) : [];
    const additions = Array.isArray(review[key]) ? review[key].map(clean).filter(Boolean) : [];
    const values = [...new Set([...previous, ...additions])].slice(0, 12);
    if (values.length > previous.length) improvedFields.push(key);
    next[key] = values;
  }
  return { observations: next, improvedFields };
}
...
function epigraphicToken(value) {
  return normalized(value).replace(/u/g, "v").replace(/[^a-z0-9]/g, "");
}

function fragmentVisible(fragment, visibleTokens) {
  const required = normalized(fragment)
    .split(/\s+/)
    .map(epigraphicToken)
    .filter((token) => token.length >= 4);
  return required.length > 0 && required.every((token) =>
    visibleTokens.some((visible) =>
      visible === token || visible.startsWith(token) || token.startsWith(visible),
    ),
  );
}

/**
 * Correct a noisy OCR field only when a curated, source-backed legend
 * signature is independently visible.  A place name or one generic word is
 * never enough; exactly one catalogue identity must satisfy its own minimum.
 */
export function applyEvidenceSignature(observations = {}, candidates = LOCAL_REFERENCE_CANDIDATES) {
  const visibleText = [
    ...
    ...(Array.isArray(observations.historicalEvidence) ? observations.historicalEvidence : []),
  ].map(clean).filter(Boolean).join(" ");
  const visibleTokens = normalized(visibleText)
    .split(/\s+/)
    .map(epigraphicToken)
    .filter((token) => token.length >= 4);
  const matches = [];
  const candidateById = new Map(
    (Array.isArray(candidates) ? candidates : []).map((candidate) => [candidate.id, candidate]),
  );
  for (const signature of Array.isArray(evidenceSignatures.signatures)
    ? evidenceSignatures.signatures
    : []) {
    const candidate = candidateById.get(clean(signature.targetCandidateId));
    if (!candidate || signature?.source?.rightsCode !== "factual-metadata-only") continue;
    const fragments = Array.isArray(signature?.requiredFragments)
      ? signature.requiredFragments.map(clean).filter(Boolean)
      : [];
    const matchedFragments = fragments.filter((fragment) =>
      fragmentVisible(fragment, visibleTokens),
    );
    const minimumMatches = Math.max(
      Number(evidenceSignatures.policy?.minimumRequiredFragments) || 4,
      Number(signature?.minimumMatches) || fragments.length,
    );
    if (fragments.length >= minimumMatches && matchedFragments.length >= minimumMatches) {
      matches.push({ candidate, signature, matchedFragments });
    }
  }
  const identities = new Set(matches.map(({ candidate }) => [
    normalized(candidate.ruler),
    clean(candidate.year),
    normalized(candidate.nominal),
    normalized(candidate.mint),
  ].join("|")));
  if (matches.length !== 1 || identities.size !== 1) {
    return { matched: false, observations, correctedFields: [], matchedFragments: [] };
  }
  const { candidate, signature, matchedFragments } = matches[0];
  const next = { ...observations };
  const correctedFields = [];
  const allowed = new Set(Array.isArray(signature.corrects) ? signature.corrects : []);
  const overrides = signature.overrides && typeof signature.overrides === "object"
    ? signature.overrides
    : {};
  for (const [target, source, correction] of [
    ["countryReading", "country", "country"],
    ["issuerReading", "issuer", "issuer"],
    ["rulerReading", "ruler", "ruler"],
    ["yearReading", "year", "year"],
    ["denominationReading", "nominal", "nominal"],
    ["mintReading", "mint", "mint"],
  ]) {
    const value = clean(overrides[correction] || candidate[source]);
    if (!allowed.has(correction) || !value || clean(next[target]) === value) continue;
    next[target] = value;
    correctedFields.push(target);
  }
  if (allowed.has("objectKind") && clean(overrides.objectKind || candidate.objectKind)) {
    next.objectKind = clean(overrides.objectKind || candidate.objectKind);
    correctedFields.push("objectKind");
  }
  next.historicalTypeHypothesis = clean(signature.label || candidate.title);
  next.historicalTypeConfidence = Math.max(
    Number(next.historicalTypeConfidence) || 0,
    94,
  );
  return {
    matched: true,
    candidate,
    signatureId: clean(signature.id),
    observations: next,
...
function rankOne(observations, candidate, inputMeasurements = {}) {
  ...
  for (const [observedKey, candidateKey, points, comparator] of fields) {
    ...
    if (
      observedKey === "rulerReading" &&
      normalized(observed).startsWith("nie dotyczy")
    ) {
      // A municipal/state issue may omit royal titulature even when the
      // catalogue records the reigning ruler. Absence is not a contradiction.
      continue;
    }
    ...
    if (comparator(observed, expected)) {
      ...
      score -= Math.round(points * 0.8);
      if (observedKey === "denominationReading") {
        // An OCR conflict may be corrected by an exact image match, but the
        // conflicting candidate can never be selected from metadata alone.
        selectionConflicts.push(
          `Odczyt nominału „${observed}” nie pasuje do „${expected}”.`,
        );
      }
    }
  }
  const unreadablePartialYear = clean(observations?.yearReading).match(
    /\b(9\d|1\d{2}|20\d)\s*(?:\?|x)/i,
  )?.[1] || "";
  if (
    unreadablePartialYear &&
    unknownObservation(observations?.yearReading) &&
    clean(candidate?.year).startsWith(unreadablePartialYear)
  ) {
    score += 20;
    reasons.push(`częściowy odczyt roku: ${unreadablePartialYear}?`);
    if (
      canonicalShape(observations?.shape) === "square-klippe" &&
      canonicalShape(candidate?.shape) === "square-klippe" &&
      selectionConflicts.some((conflict) => /nominału/i.test(conflict))
    ) {
      // Reserve a visual-comparison place for the structurally exact klippe.
      // The denomination conflict above still prevents metadata-only approval.
      score += 30;
      reasons.push("klipa z częściowym rokiem — kandydat do porównania obrazu");
    }
  }
  const observedHistorical = normalized([
    observations?.countryReading,
    observations?.issuerReading,
    observations?.historicalTypeHypothesis,
    observations?.historicalEvidence,
  ...
  const observedKlippeDesign = normalized([
    observations?.countryReading,
    observations?.portrait,
    observations?.shape,
    observations?.historicalTypeHypothesis,
  ...
  const candidateYearNumber = Number(clean(candidate?.year).match(/\b(?:19[23]\d)\b/)?.[0]);
  if (
    canonicalShape(observations?.shape) === "square-klippe" &&
    canonicalShape(candidate?.shape) === "square-klippe" &&
    normalized(candidate?.objectKind) === "pattern" &&
    clean(candidate?.periodId || candidate?.period) === "second-republic-and-war" &&
    candidateYearNumber >= 1920 &&
    candidateYearNumber <= 1939 &&
    /\bpolsk\w*\b/.test(observedKlippeDesign) &&
    /\b(?:zaglow\w*|statek|ship|sailing)\b/.test(observedKlippeDesign) &&
    selectionConflicts.some((conflict) => /roku|year|nominału|denomination/i.test(conflict)) &&
    Array.isArray(candidate?.images) &&
    candidate.images.some((url) => /^https:\/\//i.test(clean(url)))
  ) {
    // The date and nominal are one blurred digit on the reported interwar
    // klippe. Keep the small legal-image set above the visual threshold, but
    // retain both conflicts so metadata alone can never approve a candidate.
    score += 22;
    reasons.push("polska klipa międzywojenna z żaglowcem — wyłącznie do porównania obrazu");
  }
  const candidateHistorical = normalized([
    candidate?.title,
    candidate?.diagnosticMarkers,
    candidate?.obverseLegend,
    candidate?.reverseLegend,
  ].flat().filter(Boolean).join(" "));
  const observedMintFromContext = canonicalMint(observedHistorical);
  if (
    /\b(?:oblez\w*|obron\w*|siege\w*)\b/.test(observedHistorical) &&
    /\b(?:oblez\w*|obron\w*|siege\w*)\b/.test(candidateHistorical) &&
    observedMintFromContext &&
    sameMint(observedMintFromContext, candidate?.mint)
  ) {
    // Municipal siege issues do not necessarily name or portray a monarch.
    // This score only lets the correct city/type reach the visual gate; a
    // conflicting year remains in selectionConflicts and cannot be approved
    // from metadata alone.
    score += 36;
    reasons.push(`miejska emisja oblężnicza: ${observedMintFromContext}`);
  }
...
function canonicalShape(value) {
  ...
  if (/\b(klip|klipa|klippe|square|kwadrat|romb|diamond|lozenge|czworokat)/.test(shape)) {
    return "square-klippe";
  }
...
function literalDenominationFromEvidence(observations = {}) {
  const visibleText = [
    observations.denominationEvidence,
    ...(Array.isArray(observations.obverseLegendFragments)
      ? observations.obverseLegendFragments
      : []),
    ...(Array.isArray(observations.reverseLegendFragments)
    ...
    .join(" ");
  const found = new Set();
  for (const match of normalized(visibleText).matchAll(
    /\b(\d{1,3})\s*(zl|zloty|zlote|zlotych|gr|grosz|grosze|groszy)\b/g,
  )) {
    const amount = String(Number(match[1]));
    const unit = match[2].startsWith("zl") ? "zł" : "gr";
    found.add(`${amount} ${unit}`);
  }
  return found.size === 1 ? [...found][0] : "";
}

/**
 * Prefer a single denomination quoted from the photographed legend over the
 * model's remembered coin type. This is intentionally narrow: without a unit,
 * or when two values are visible, no correction is made.
 */
export function reconcileDirectDenominationEvidence(observations = {}) {
  const literal = literalDenominationFromEvidence(observations);
  if (!literal) return { ...observations };
  const current = clean(observations.denominationReading);
  if (!unknownObservation(current) && sameNominal(current, literal)) {
    return { ...observations };
  }
  return {
    ...observations,
    denominationReading: literal,
  };
}
...
function sameYear(value, expected) {
  ...
  const partialPrefix = clean(value).match(/\b(9\d|1\d{2}|20\d)\s*(?:\?|x)/i)?.[1] || "";
  if (partialPrefix && String(referenceYear).startsWith(partialPrefix)) return true;
...
function unresolvedByMeasurement(selected, candidates, measurements, observations = {}) {
  if (measurements.weightGrams) return null;
  const visibleNominal = literalDenominationFromEvidence(observations);
  if (
    visibleNominal &&
    clean(selected.nominal) &&
    sameNominal(visibleNominal, selected.nominal)
  ) return null;
  ...
  const historicalType = normalized(observations.historicalTypeHypothesis);
  const historicalEvidence = Array.isArray(observations.historicalEvidence)
    ? observations.historicalEvidence.map(clean).filter(Boolean)
    : [];
  const selectedNominal = normalized(selected.nominal);
  const rivalNominal = normalized(rival.nominal);
  const namesSelectedNominal = selectedNominal && new RegExp(
    `(?:^|\\s)${selectedNominal.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:\\s|$)`,
  ).test(historicalType);
  const namesRivalNominal = rivalNominal && new RegExp(
    `(?:^|\\s)${rivalNominal.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:\\s|$)`,
  ).test(historicalType);
  if (
    Number(observations.historicalTypeConfidence) >= 80 &&
    historicalEvidence.length >= 2 &&
    namesSelectedNominal &&
    !namesRivalNominal
  ) return null;
  return {
    rival,
...
function literalVisibleValue(field, observed, observations = {}) {
  ...
  if (["country", "issuer", "depictedPerson", "period", "mint"].includes(field)) {
    ...
    if (value.length <= 120 && evidence.length >= 2 && hypothesisConfidence >= 70) {
...
export function adjudicateRecognition(raw, candidates, inputMeasurements = {}) {
  ...
  const measurementAmbiguity = unresolvedByMeasurement(
    selected,
    candidates,
    measurements,
    checkedObservations,
  ...
  if (weightConflict) followUpQuestions.push("Zważ monetę z dokładnością co najmniej 0,1 g.");
