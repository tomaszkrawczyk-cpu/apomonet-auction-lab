...
function publicLocalRecord(record) {
  ...
  return {
    ...record,
    ruler: clean(record.ruler) || (inferredRuler && inferredRuler !== "polska" ? inferredRuler : ""),
    ...hierarchy,
...
function rankOne(observations, candidate, inputMeasurements = {}) {
  ...
  if (
    ...
    if (observedObjectKind === candidateObjectKind) {
      ...
      score -= 60;
      hardConflicts.push(
        `Rodzaj obiektu „${observedObjectKind}” nie pasuje do „${candidateObjectKind}”.`,
      );
    }
  }
  const fields = [
...
function rulerReadingFromEvidence(observations = {}) {
  ...
  return unknownObservation(reading) ? legend : reading;
}
...
function literalVisibleValue(field, observed) {
...
function partialIdentityFromEvidence({
  ...
  for (const [field, rule] of Object.entries(PARTIAL_FIELD_RULES)) {
    ...
    // A literal date or denomination visible on the object remains useful
    // evidence even when the current catalogues have no compatible record.
    // Preserve that observation without turning it into a positive identity.
    if (!reference && rule.observation) {
      const literal = literalVisibleValue(field, observed);
      if (literal) {
...
export function adjudicateRecognition(raw, candidates, inputMeasurements = {}) {
  ...
  const proposedHardConflicts = Array.isArray(proposedRanking?.hardConflicts)
    ...
    : [];
  const selected = proposedRanking && proposedRanking.hardConflicts.length === 0
    ? proposedSelected
    : null;
  ...
  if (
    ...
    return {
      ...
      partialIdentity: {
        ...
        fieldConfidence: { objectKind: 100 },
        basis: { objectKind: "visual-object-class" },
        populatedFields: ["objectKind"],
      },
    };
  }
  ...
  if (!selected) {
    ...
    return {
      ...
      selected: null,
      observations: checkedObservations,
      contradictions: proposedHardConflicts.length
        ? [...new Set(proposedHardConflicts)]
...
export function analysisFromRecognition(raw, recognition, condition) {
  ...
  const depictedPerson = /chryst|christ|salvator|jezus|jesus/.test(normalizedPortrait)
    ? "Chrystus"
  ...
  return {
    ...
    issuer: municipalIssuer || candidate?.ruler || partial.ruler || UNKNOWN,
    ruler: candidate?.ruler || (municipalIssuer ? "Nie dotyczy — emisja miejska" : partial.ruler || UNKNOWN),
    ...
    portraitRuler: confirmed
      ? candidate.ruler || (municipalIssuer ? "Nie dotyczy — emisja miejska" : UNKNOWN)
      : partial.ruler || UNKNOWN,
