...
function evidenceQuery(observations = {}) {
  ...
  const useful = [ruler, year, mint].filter(
    (value) => value && normalized(value) !== "nie ustalono",
  );
  if (useful.length < 2 && nominal && normalized(nominal) !== "nie ustalono") useful.push(nominal);
...
function rankOne(observations, candidate) {
  ...
  const fields = [
    ...
    ["yearReading", "year", 20, (value, expected) => numericYear(value) === numericYear(expected)],
    ["denominationReading", "nominal", 25, sameNominal],
    ["mintReading", "mint", 20, same],
  ...
  for (const [observedKey, candidateKey, points, comparator] of fields) {
    ...
    if (!observed || !expected || normalized(observed) === "nie ustalono") continue;
...
function unresolvedByMeasurement(selected, candidates, measurements) {
  ...
  const rival = candidates.find(
    (candidate) =>
      ...
      same(candidate.mint, selected.mint) &&
      normalized(candidate.shape) === normalized(selected.shape) &&
...
export function adjudicateRecognition(raw, candidates, inputMeasurements = {}) {
  ...
  const checks = [
    ...
    ["yearReading", "year", 15, (value, expected) => numericYear(value) === numericYear(expected)],
    ["denominationReading", "nominal", 16, sameNominal],
    ["mintReading", "mint", 10, same],
    ["shape", "shape", 7, same],
  ];
...
export function conditionFromRaw(raw, imageUsable = true) {
  ...
  return {
    ...
    band: bands[code] || UNKNOWN,
    bandCode: bands[code] ? code : "uncertain",
    confidence,
...
export function analysisFromRecognition(raw, recognition, condition) {
  ...
  return {
    ...
    warnings: recognition.contradictions.slice(0, 4),
    estimateLow: 0,
    estimateHigh: 0,
