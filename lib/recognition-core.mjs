...
function namedPortrait(value) {
  ...
  if (/\b(?:portret|popiers\w*|profil|twarz|glow\w*|wizerunek|w koronie|orzel|herb|postac)\b/.test(token)) return "";
...
function semanticCandidateRoles(record = {}) {
  ...
  return {
    ...
    ruler,
    mint,
    depictedPerson: clean(record.depictedPerson) || namedPortrait(record.portrait),
  };
}
...
function literalVisibleValue(field, observed, observations = {}) {
  ...
  if (field === "ruler" && explicitRuler(value)) return value;
  if (["country", "issuer", "depictedPerson", "period", "mint"].includes(field)) {
    ...
    if (value.length <= 120 && evidence.length >= 2 && hypothesisConfidence >= 75) return value;
  }
  return "";
}
...
export function analysisFromRecognition(raw, recognition, condition) {
  ...
  const partialSummary = [partial.country, partial.ruler, partial.depictedPerson, partial.nominal, partial.year, partial.mint]
    .filter(Boolean)
    .join(", ");
  ...
  const depictedPerson = !unknownObservation(directlyReadPerson)
    ? directlyReadPerson
