...
const LOCAL_REFERENCE_CANDIDATES = buildLocalReferenceCandidates();
...
export function analysisFromRecognition(raw, recognition, condition) {
  ...
  return {
    ...
    analysisLevel: "basic",
    analysisVersion: "retrieval-first-v3",
    recognition: {
      engineVersion: "retrieval-first-v3",
...
export const recognitionCatalogPolicy = Object.freeze({
  schemaVersion: "apomonet-recognition-catalog-set-v1",
  ...
  allPatternCandidateCount: LOCAL_REFERENCE_CANDIDATES.filter(
    (record) => record.objectKind === "pattern" || record.coinClass === "pattern" || /\bprobn\w*/.test(normalized(record.title)),
  ).length,
