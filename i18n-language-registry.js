(()=>{
  ...
  function current(){return normalize(window.ApoI18n?.current?.()||localStorage.getItem(KEY)||document.documentElement.lang||'pl')}
  ...
  function isEnabled(code){return registry.has(normalize(code))}
  ...
  function resolve(table,key,code=current()){const row=table?.[key];if(row==null)return key;if(typeof row==='string')return row;for(const lang of chain(code)){const v=lang==='pl'?(row.pl??key):row[lang];if(v!==undefined&&v!==null&&String(v).trim()!=='')return v}return key}
  function languages(){return [...registry.values()]}
  window.ApoLanguageRegistry={register,get,isEnabled,current,chain,resolve,languages,normalize};
})();
