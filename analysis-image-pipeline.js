(()=>{
  ...
  const L={
    pl:{noFile:'Nie wybrano zdjęcia.',tooLarge:'Zdjęcie jest zbyt duże. Wybierz plik mniejszy niż 30 MB.',readFail:'Nie udało się odczytać zdjęcia.',unsupported:'Plik nie jest obsługiwanym zdjęciem.',badSize:'Zdjęcie ma nieprawidłowe wymiary.',shrinkFail:'Nie udało się bezpiecznie zmniejszyć zdjęcia. Spróbuj wykonać je ponownie z nieco mniejszej odległości.',note:'Podgląd zachowuje całe oryginalne zdjęcie. Bezpieczny kadr jest tworzony osobno wyłącznie do analizy i nigdy nie zastępuje zdjęcia zapisywanego z monetą.'},
    en:{noFile:'No photo selected.',tooLarge:'The photo is too large. Choose a file smaller than 30 MB.',readFail:'The photo could not be read.',unsupported:'The file is not a supported image.',badSize:'The photo has invalid dimensions.',shrinkFail:'The photo could not be reduced safely. Try taking it again from a slightly shorter distance.',note:'The preview keeps the complete original photo. A safe crop is prepared separately for analysis and never replaces the photo saved with the coin.'},
    de:{noFile:'Kein Foto ausgewählt.',tooLarge:'Das Foto ist zu groß. Wählen Sie eine Datei unter 30 MB.',readFail:'Das Foto konnte nicht gelesen werden.',unsupported:'Die Datei ist kein unterstütztes Bild.',badSize:'Das Foto hat ungültige Abmessungen.',shrinkFail:'Das Foto konnte nicht sicher verkleinert werden. Nehmen Sie es erneut aus etwas geringerer Entfernung auf.',note:'Die Vorschau behält das vollständige Originalfoto. Ein sicherer Ausschnitt wird nur für die Analyse erstellt und ersetzt nie das mit der Münze gespeicherte Foto.'},
    fr:{noFile:'Aucune photo sélectionnée.',tooLarge:'La photo est trop volumineuse. Choisissez un fichier de moins de 30 Mo.',readFail:'Impossible de lire la photo.',unsupported:'Le fichier n’est pas une image prise en charge.',badSize:'La photo a des dimensions incorrectes.',shrinkFail:'Impossible de réduire la photo en toute sécurité. Reprenez-la d’un peu plus près.',note:'L’aperçu conserve la photo originale entière. Un cadrage sûr est créé séparément pour l’analyse et ne remplace jamais la photo enregistrée avec la monnaie.'}
  };
  ...
  async function processCoin(file){const image=await loadImage(file),source=scaledCanvas(image,WORK_MAX),detection=detectCircle(source),rect=safeCrop(source,detection),displayRect=fullFrame(source),quality=assessPhoto(detection,rect),displayCanvas=render(source,displayRect,DISPLAY_MAX),analysisCanvas=render(source,rect,ANALYSIS_MAX),minSide=Math.min(source.width,source.height);return{display:boundedDataUrl(displayCanvas,'image/webp',.74),analysis:boundedDataUrl(analysisCanvas,'image/jpeg',.62),confidence:detection.confidence,edgeScore:detection.score,backgroundTexture:detection.backgroundTexture,cropMode:rect.mode,previewMode:displayRect.mode,quality:quality.level,qualityReason:quality.reason,coinCoverage:minSide?Number((detection.r*2/minSide).toFixed(3)):0,centerOffset:minSide?Number((Math.hypot(detection.cx-source.width/2,detection.cy-source.height/2)/minSide).toFixed(3)):1,analysisWidth:analysisCanvas.width,analysisHeight:analysisCanvas.height}}
  ...
  window.ApoImagePipeline={processCoin,detectCircle,safeCrop,fullFrame,assessPhoto,boundedDataUrl,install};
