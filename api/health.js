...
module.exports=async function handler(req,res){
 ...
 if(req.method==='POST'){
  const body=req.body||{};if(body.mode!=='catalog-candidate')return res.status(400).json({error:'Unsupported mode'});
  const key=process.env.OPENAI_API_KEY?.trim();if(!key)return res.status(500).json({error:'Brak OPENAI_API_KEY'});
  const base=body.base&&typeof body.base==='object'?body.base:{},detail=body.detail&&typeof body.detail==='object'?body.detail:{},images=Array.isArray(body.images)?body.images.filter(x=>typeof x==='string'&&x.startsWith('data:image/')).slice(0,2):[];
  ...
  const data=await r.json();if(!r.ok)return res.status(r.status).json({error:data?.error?.message||'Błąd analizy katalogowej'});const txt=textFrom(data);if(!txt)return res.status(502).json({error:'Pusty wynik katalogowy'});
