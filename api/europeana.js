export default async function handler(req,res){
 ...
 try{
  ...
  const o=j.object||{};
  const first=x=>Array.isArray(x)?String(x[0]??''):String(x??'');
  const langFirst=x=>{if(!x)return'';if(typeof x==='string')return x;if(Array.isArray(x))return first(x);for(const k of ['pl','en','def'])if(x[k])return first(x[k]);const v=Object.values(x)[0];return first(v)};
  const rights=[];
  for(const a of [...(o.aggregations||[]),...(o.europeanaAggregation||[])]){const v=a?.edmRights||a?.edmrights;if(typeof v==='string')rights.push(v);else if(Array.isArray(v))rights.push(...v.map(String));else if(v&&typeof v==='object')for(const x of Object.values(v))Array.isArray(x)?rights.push(...x.map(String)):rights.push(String(x))}
  const provider=first(o.provider)||langFirst(o.aggregations?.[0]?.edmDataProvider)||langFirst(o.aggregations?.[0]?.edmProvider);
  const item={source:'EUROPEANA',id:o.about||id,uri:'https://www.europeana.eu/item'+id,label:first(o.title)||id,conceptType:first(o.type),year:first(o.year),provider,rightsStatement:[...new Set(rights.filter(Boolean))].join(' | '),license:'METADATA_CC0',fetchedAt:new Date().toISOString()};
