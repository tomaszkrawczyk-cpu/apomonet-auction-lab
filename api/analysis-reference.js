...
function nominalForms(v){const x=norm(v);if(/poltalar|1 2 talar|half thaler/.test(x))return['half thaler','1/2 thaler','halber taler','poltalar'];if(/dwutalar|2 talar/.test(x))return['2 taler','2 talar','dwutalar'];if(/talar/.test(x))return['taler','thaler','talar'];if(/dukat/.test(x))return['ducat','dukat'];if(/zlot/.test(x))return['zloty','zlote','zlotych'];if(/grosz|grosh/.test(x))return['grosz','grosh','groszy'];return x?[x]:[]}
