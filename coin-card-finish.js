(()=>{
  ...
  addEventListener('DOMContentLoaded',()=>{
    ...
    style.textContent=`
      ...
      .coin-content .photo-card{padding:12px}.coin-content .photo-card h2{font-size:17px;margin:0 0 10px}
    ...
    if(!document.querySelector('.apo-saved-banner')){
      const banner=document.createElement('section');banner.className='apo-saved-banner';banner.innerHTML='<strong>✓ Zapisano do kolekcji</strong><span>Zdjęcia i zaakceptowane dane tej monety są zapisane razem.</span>';content.prepend(banner);
    }

    const detail=current.detail;
    const trust=window.ApoKnowledgeTrust?.level?.(current)||{label:current.userAccepted?'Korekta użytkownika':'Sugestia AI',rank:current.userAccepted?30:10};
    const card=document.createElement('section');card.className='card apo-detail-card';
    const h=document.createElement('div');h.className='section-title';const title=document.createElement('h2');title.textContent=detail?'Analiza szczegółowa':'Status rekordu';const badge=document.createElement('span');badge.className='apo-trust';badge.dataset.rank=String(trust.rank||10);badge.textContent=trust.label||'Sugestia AI';h.append(title,badge);card.appendChild(h);
    const grid=document.createElement('div');grid.className='apo-detail-grid';
    const add=(label,value,{wide=false,warning=false}={})=>{const text=String(value??'').trim();if(!text)return;const box=document.createElement('div');box.className='apo-detail-item'+(wide?' apo-detail-wide':'');const l=document.createElement('span');l.textContent=label;const p=document.createElement('p');p.textContent=text;if(warning)p.className='apo-catalog-warning';box.append(l,p);grid.appendChild(box)};
    if(detail){
      add('Odmiana / wariant',detail.variant||current.variant);
      const catalog=[detail.kopickiReference,detail.kopickiRarity].filter(Boolean).join(' • ');add('Kopicki / rzadkość',catalog||'Niepotwierdzone',{warning:!catalog});
      add('Awers — obserwacje',detail.obverseDetails,{wide:true});add('Rewers — obserwacje',detail.reverseDetails,{wide:true});
      add('Legenda awersu',detail.obverseLegend,{wide:true});add('Legenda rewersu',detail.reverseLegend,{wide:true});
      add('Cechy diagnostyczne',Array.isArray(detail.diagnosticFeatures)?detail.diagnosticFeatures.join('; '):detail.diagnosticFeatures,{wide:true});
      add('Stan — ocena ostrożna',detail.gradeAssessment,{wide:true});
      add('Co może rozstrzygnąć wynik',Array.isArray(detail.recommendedChecks)?detail.recommendedChecks.join(' '):detail.recommendedChecks,{wide:true});
      add('Uwagi i ograniczenia',Array.isArray(detail.warnings)?detail.warnings.join(' '):detail.warnings,{wide:true,warning:true});
    }
    card.appendChild(grid);
    const toolbar=content.querySelector('.toolbar');if(toolbar)content.insertBefore(card,toolbar);else content.appendChild(card);
  });
})();
