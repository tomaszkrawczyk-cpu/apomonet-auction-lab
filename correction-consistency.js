(() => {
  ...
  function reanalysisMessage(){const language=window.ApoLanguageRegistry?.current?.()||window.ApoI18n?.current?.()||localStorage.getItem('apomonet_language_v2')||'pl';return({pl:'Wymaga ponownej analizy po korekcie danych identyfikacyjnych.',en:'Requires reanalysis after identity correction.',de:'Erfordert eine erneute Analyse nach der Identitätskorrektur.',fr:'Nécessite une nouvelle analyse après correction de l’identification.'}[language]||'Requires reanalysis after identity correction.')}
  function repairExport(){if(!location.pathname.endsWith("export.html"))return;normalizeState();const ids=JSON.parse(sessionStorage.getItem("apomonet_export_ids")||"[]"),state=ApoMonet.load();let coins=(state.coins||[]).filter((coin)=>ids.includes(coin.id));if(!coins.length){try{coins=JSON.parse(sessionStorage.getItem("apomonet_demo_export_coins")||"[]")}catch{coins=[]}}document.querySelectorAll(".export-card").forEach((card,index)=>{const coin=normalizeCoin(coins[index]);if(!coin)return;const heading=card.querySelector("h2");if(heading)heading.textContent=coin.canonicalTitle||coin.title||"Coin";const valuationCell=card.querySelector(".market-grid > div:first-child");if(valuationCell&&coin.needsReanalysis){const label=valuationCell.querySelector("span"),labelText=label?.textContent||"Valuation";valuationCell.replaceChildren();const span=document.createElement("span");span.textContent=labelText;valuationCell.append(span,document.createTextNode(reanalysisMessage()));}});}
  ...
  function init(){installWriteGuard();normalizeState();afterSubmit();repairExport();if(location.pathname.endsWith("export.html")){["languagechange","apo-language-changed","apomonet:language-change"].forEach((eventName)=>addEventListener(eventName,repairExport));setTimeout(repairExport,120)}}
