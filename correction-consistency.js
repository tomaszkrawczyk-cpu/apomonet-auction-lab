(() => {
  ...
  function known(value) {
    const x = comparable(value);
    return Boolean(x) && !UNKNOWN_VALUES.has(x);
  }
  ...
  function reconcileDescription(coin) {
    ...
    const source = clean(coin.fullDescription || coin.description || coin.rawAI?.fullDescription || coin.rawAI?.description);
    ...
    const observations = splitSentences(source).filter((sentence,index)=>!shouldDiscardSentence(sentence,coin,changed,index));
  ...
  function normalizeCoin(coin) {
    ...
    if (/data nie została odczytana|nominał.*wymaga|konfliktu rok|wstrzymana/i.test(clean(normalized.valuationNote))) {
      normalized.estimateLow = 0; normalized.estimateHigh = 0; normalized.estimatedPrice = ""; normalized.priceRange = "";
      normalized.valuationNote = "Wycena wymaga ponownej analizy po korekcie danych zaakceptowanych przez użytkownika.";
    }
    return normalized;
  }
  ...
  function installWriteGuard() {
    ...
    ApoMonet.upsertCoin = function (coin) { return original.call(ApoMonet, normalizeCoin(coin)); };
  ...
  function normalizeState() {
    ...
    state.coins = (state.coins || []).map((coin) => { const normalized=normalizeCoin(coin); if(JSON.stringify(normalized)!==JSON.stringify(coin))changed=true; return normalized; });
    if (changed) ApoMonet.save(state);
  }

  function syncAnalysisSession(coin) {
    if (!coin) return;
    let old={}; try{old=JSON.parse(sessionStorage.getItem("apomonetAnalysisSession")||"{}")||{}}catch{}
    const corrected={...(old.a||{}),...coin,description:coin.description||old.a?.description||"",fullDescription:coin.fullDescription||coin.description||old.a?.fullDescription||old.a?.description||"",userAccepted:true,correctedAt:new Date().toISOString()};
    const session={...old,id:coin.id,a:corrected,imgs:Array.isArray(old.imgs)?old.imgs:[coin.obverseImage||null,coin.reverseImage||null],analysisImgs:Array.isArray(old.analysisImgs)?old.analysisImgs:[coin.obverseImage||null,coin.reverseImage||null],photoDiagnostics:Array.isArray(old.photoDiagnostics)?old.photoDiagnostics:[null,null],at:Date.now(),version:Math.max(Number(old.version)||0,5)};
    try{sessionStorage.setItem("apomonetAnalysisSession",JSON.stringify(session))}catch(error){console.warn("[correction-session-sync]",error)}
  }

  function repairExport() {
    if (!location.pathname.endsWith("export.html")) return;
    normalizeState();
    const ids=JSON.parse(sessionStorage.getItem("apomonet_export_ids")||"[]"),state=ApoMonet.load();
    let coins=(state.coins||[]).filter((coin)=>ids.includes(coin.id));
    if(!coins.length){try{coins=JSON.parse(sessionStorage.getItem("apomonet_demo_export_coins")||"[]")}catch{coins=[]}}
    document.querySelectorAll(".export-card").forEach((card,index)=>{
      const coin=normalizeCoin(coins[index]); if(!coin)return;
      const heading=card.querySelector("h2"); if(heading)heading.textContent=coin.canonicalTitle||coin.title||"Coin";
      const cells=[...card.querySelectorAll(".data-grid div")];
      const valuationCell=cells[10];
      if(valuationCell&&coin.needsReanalysis){
        const label=valuationCell.querySelector("span");
        const labelText=label?.textContent||"Valuation";
        valuationCell.replaceChildren();
        const span=document.createElement("span"); span.textContent=labelText;
        valuationCell.append(span,document.createTextNode("Requires reanalysis after user-corrected identification data."));
      }
    });
  }
  ...
  function afterSubmit() {
    ...
    form.addEventListener("submit",()=>setTimeout(()=>{normalizeState();const id=new URLSearchParams(location.search).get("id"),coin=id?ApoMonet.getCoin(id):null;if(coin){syncAnalysisSession(coin);const title=document.getElementById("title"),description=document.getElementById("description");if(title&&coin.canonicalTitle)title.value=coin.canonicalTitle;if(description&&coin.description)description.value=coin.description}},0));
    document.addEventListener("click",event=>{const target=event.target?.closest?.("#backResult,#chooseAlbum,#openCard");if(!target)return;normalizeState();const id=new URLSearchParams(location.search).get("id"),coin=id?ApoMonet.getCoin(id):null;if(coin)syncAnalysisSession(coin)});
  }

  window.ApoCorrectionConsistency=Object.freeze({changedFields,reconcileDescription,normalizeCoin,normalizeState,syncAnalysisSession});
