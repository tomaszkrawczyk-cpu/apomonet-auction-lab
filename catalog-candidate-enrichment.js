(()=>{
  ...
  const requestPath=input=>{try{return new URL(typeof input==='string'?input:input?.url||'',location.href).pathname}catch{return''}};
  ...
  window.fetch=async(input,init)=>{
    ...
    if(requestPath(input)!=='/api/analyze-detail'||!response.ok)return response;
    try{
  ...
  window.ApoCatalogCandidateEnrichment=Object.freeze({eligibleCountry,sessionId,persistCandidate,requestPath});
})();