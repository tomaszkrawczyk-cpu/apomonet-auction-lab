(()=>{
  ...
  function buyerCost(r){
    const hammer=Number(r?.hammerPrice)||0;if(!hammer||!window.ApoAuctionCost?.calculateForHouse)return null;
    const house=r.auctionHouse||r.sourceLabel||r.source||'';const cost=ApoAuctionCost.calculateForHouse({hammer,auctionHouse:house,saleDate:r.soldAt,currency:r.currency});
    return cost?.complete?cost:null;
  }
  function render(){
