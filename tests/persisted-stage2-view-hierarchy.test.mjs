...
test('export view prefers saved accepted fields then persisted Stage 2 detail and never raw AI',()=>{
  ...
  assert.match(source,/PRIVATE_FIELDS=\['rawAI'/);
  assert.match(source,/for\(const key of PRIVATE_FIELDS\)delete output\[key\]/);
  ...
  const coin=normalize({variant:'accepted variant',description:'accepted description',detail:{variant:'stage2 variant',catalogEvidenceStatus:'supported-by-stage2-variant-evidence',kopickiReference:'K. 1',kopickiRarity:'R2',fullDescription:'stage2 description'},rawAI:{variant:'raw variant',fullDescription:'raw description'}});
  ...
  assert.equal(coin.rawAI,undefined);
});
