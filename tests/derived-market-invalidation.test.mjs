...
test('identity corrections invalidate stale market and catalog data',()=>{
  for(const field of ['detail','kopickiReference','kopickiRarity','auctionRecords10y','auctionStrictMatches10y','marketMedian','marketCurrency','valuationCurrency','priceRange','valuationConfidence','valuationUpdatedAt','estimateLow','estimateHigh','estimatedPrice','estimatedValue','marketValue','valuation','priceEstimate','estimate','auctionMarketIdentityKey','correctionReanalysisIdentityKey','marketReanalysisCompletedAt']){
  ...
  assert.match(source,/output\.valuationNote =\s*"Wycena, literatura i notowania wymagają ponownego dopasowania po korekcie identyfikacji\."/);
});
