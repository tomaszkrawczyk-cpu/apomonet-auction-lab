...
test('condition normalizer stays broad and conservative',()=>{
  assert.match(grades,/F:1,VF:2,XF:3,AU:4/);
  assert.doesNotMatch(grades,/MS6[0-9]|AU5[0-9]/);
});
test('same condition is preferred and distant condition is penalized',()=>{
  assert.match(core,/conditionDistance===0/);
  assert.match(core,/conditionDistance!=null&&conditionDistance>=2/);
