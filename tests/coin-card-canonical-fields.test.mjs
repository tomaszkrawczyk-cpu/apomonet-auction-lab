...
test('canonical coin card layer is loaded after side integrity and duplicate renderer is disabled',()=>{
...
test('coin card only exposes confirmed fresh catalog references',()=>{
  ...
  assert.match(src,/raritySource==='catalog-candidate'/);
});
test('canonical renderer owns variant rarity and catalog facts',()=>{
  assert.match(src,/set\(5,v\.variant\)/);
  assert.match(src,/set\(7,v\.rarity\)/);
  assert.match(src,/set\(8,v\.catalog\)/);
});
