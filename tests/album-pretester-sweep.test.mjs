...
test('canonical album UI suppresses legacy cards and owns the new-album action',()=>{
  const src=read('user-albums-ui.js');
  assert.match(src,/\[data-custom-album\]/);
  assert.match(src,/suppressLegacyAlbumHotfix/);
  assert.match(src,/apoLegacyAlbumLabel/);
