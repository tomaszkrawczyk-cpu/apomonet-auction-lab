(()=>{
  ...
  const exportPhoto=(coin,side)=>{if(coin?.albumPhotoMode==='none')return'';if(coin?.albumPhotoMode==='cut'&&Number(coin.albumPhotoPrepVersion||0)>=2){const cut=side==='obverse'?clean(coin.albumObverseImage):clean(coin.albumReverseImage);return cut||originalPhoto(coin,side)}return originalPhoto(coin,side)};
